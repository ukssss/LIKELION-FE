...
export function insertLast(node, text) {
  if (typeof node === "string") node = getNode(node);
  if (node.nodeType !== document.ELEMENT_NODE) refError("insertLast 함수의 첫 번째 인자는 ELEMENT 노드여야 합니다");
  node.insertAdjacentHTML("beforeend", text);
}
function insertAfter(node, text) {
  if (typeof node === "string") node = getNode(node);
  if (node.nodeType !== document.ELEMENT_NODE) refError("insertAfter 함수의 첫 번째 인자는 ELEMENT 노드여야 합니다");
